...
<div class="row p-4 bg-gray">
    <div class="col-lg-2">
        <div class="form-group">
            <label for="name">{{ trans('tenant::text.tenant name') }}</label>
            {{ Form::text('name', request('name'), ['class' => 'form-control', 'id' => 'name']) }}
        </div>
    </div>
    <div class="col-lg-2">
        <div class="form-group">
            <label for="email">{{ trans('tenant::text.tenant email') }}</label>
            {{ Form::text('email', request('email'), ['class' => 'form-control', 'id' => 'email']) }}
        </div>
    </div>
    <div class="col-lg-2">
        <div class="form-group">
            <label for="email">{{ trans('tenant::text.tenant phone') }}</label>
            {{ Form::text('phone', request('phone'), ['class' => 'form-control', 'id' => 'phone']) }}
        </div>
    </div>
</div>
