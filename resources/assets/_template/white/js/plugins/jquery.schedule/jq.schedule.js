...
(function($) {
    $.fn.timeSchedule = function(options){
        var defaults = {
            ...
            headTimeBorder:1,	// time border widthf
        ...
        this.addItemSchedule = function (data) {
            element.addScheduleData(data);
            element.resetBarPosition(data['timeline']);
        };
        // スケジュール追加
        this.addScheduleData = function(data){
            data["start"] = element.calcStringTime(data["start"]);
            data["end"] = element.calcStringTime(data["end"]);

            var st = Math.ceil((data["start"] - tableStartTime) / setting.widthTime);
            var et = Math.floor((data["end"] - tableStartTime) / setting.widthTime);
            var $bar = jQuery('<div class="sc_Bar" data-toggle="tooltip" data-container="body" data-html="true" data-placement="right" title =""><span class="head"><span class="time"></span></span><div class="text" data-toggle="tooltip" data-container="body" data-html="true" data-placement="right" title =""></div></div>');
            var stext = element.formatTime(data["start"]);
            var etext = element.formatTime(data["end"]);
            var snum = element.getScheduleCount(data["timeline"]);
            $bar.css({
            ...
            if(typeof data.class != 'undefined'){
                $bar.css('background', `#${data.class}`);
            }
            if(data.data.memo === true){
                $bar.find(".head").before('<div class="memo-icon"><i class="fa fa-comment-o"></i></div>');
            }
            if(data.data.memo === true){
                $bar.find(".head").before('<div class="question-icon"><i class="fa fa-question-circle-o"></i></div>');
            }

            if(data["text"]){
                $bar.find(".text").html(data["text"]);
                // $bar.find(".text").attr('title', data["text"].replace(/<[^>]*>?/gm, ''));
                $bar.find(".text").attr('title', data["text"].replace(/<[^>]*>?/gm, ' '));
                $bar.attr('title', data["text"].replace(/<[^>]*>?/gm, ' '));
            }
            //$element.find('.sc_main').append($bar);
            $element.find('.sc_main .timeline').eq(data["timeline"]).append($bar);
            // データの追加
            ...
            $node.draggable({
                ...
                start: function(event, ui) {
                    ...
                    currentNode = node;
                    moveFormLine = timelineData[scheduleData[jQuery(this).data("sc_key")]['timeline']];
                },
                ...
                stop: function(event, ui) {
                    ...
                    if(setting.change){
                        setting.change(node, scheduleData[sc_key], timelineData[scheduleData[sc_key]['timeline']], moveFormLine);
                    }
                }
            });
            $node.resizable({
        ...
        this.addRow = function(timeline,row){
            ...
            for(var t=tableStartTime;t<tableEndTime;t+=setting.widthTime){
                ...
                $tl.addClass('scale-hover' + scaleClass);
                if(setting.scaleTime){
                    $tl.css('background-color', '#ffffff');
                }

                if (countCss % 3 == 0) {
                    if (countCss > 0) {
                        $tl.css('border-left', '1px solid rgb(222, 215, 215)');
                    }
                } else {
                    if (setting.scaleTime) {
                        $tl.css('border-left', '1px solid transparent');
                    } else {
                        $tl.css('border-left', '1px dotted #dedede');
                    }
                }

                if(t + setting.widthTime == tableEndTime){
                    $tl.css('border-right', '1px solid #dedede');
                }
            ...
            if(setting.time_click){
                $timeline.find(".tl").on('touch click',function(){
                    if(setting.scaleTime){
                        var hour = jQuery(this).data("time").split(':')[0];
                        var minutes = jQuery(this).data("time").split(':')[1];
                        if(minutes % 15 != 0){
                            while(minutes % 15 != 0){
                        ...
                        setting.time_click(this, hour + ":" + minutes,
                            jQuery(this).data("timeline"),timelineData[jQuery(this).data("timeline")]);
                    }else {
                        setting.time_click(this,jQuery(this).data("time"),
                            jQuery(this).data("timeline"),timelineData[jQuery(this).data("timeline")]);
                    }
                });
            }
            ...
            //scaleTimeHover
            if(setting.scaleTime){
                if(setting.hover_bind){
                    $timeline.find(".tl").on('mouseenter', function(){
                        let className = $(this).attr('class').split(" ");
                        $timeline.find('.' + className[1]).css('background', '#F0F0F0');
                    }).on('mouseleave', function(){
                        let className = $(this).attr('class').split(" ");
                        $timeline.find('.' + className[1]).css('background', '#fff');
                    });
                }
            }
