...
(function(factory) {
    ...
    var PagesCalendar = function(element, options) {
        ...
        wView.prototype.render = function() {
            ...
            this.startOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf('isoWeek');
            if(this.weekStart >= this.weekEnd)
                this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf('isoWeek').add("days", 7 + parseInt(Calendar.settings.ui.week.endOfTheWeek));
            else
                this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf('isoWeek').add("days",parseInt(Calendar.settings.ui.week.endOfTheWeek));
        ...
        wView.prototype._buildLayout = function() {
            ...
            let startTime = this.calcStringTime(Calendar.settings.minTime);
            let endTime = this.calcStringTime(Calendar.settings.maxTime);
            let before_time = -1;

            for (let t = startTime; t < endTime; t += Calendar.settings.widthTime) {
                if((before_time < 0) || (Math.floor(before_time / 3600) != Math.floor(t / 3600))){
                    Calendar.content += '<div class="trow" >';
                    var _slotCount = 12;
                    var _maxWidth = 80;
                    var minutes = this.formatTime(t);
                    var minutesGet = minutes.split(':');
                    let minutesTimeLimit = Calendar.settings.maxTime.split(':');

                    if(minutesGet[1] != '00' || (minutesTimeLimit[0] == minutesGet[0] && minutesTimeLimit[1] != '00')){
                        _slotCount = (30 /60) * 12;
                        _maxWidth = (30 / 60) * 80;
                    }

                    for (var j = this.weekStart; j <= weekend; j++) {
                        Calendar.content += '<div class="tcell" style="height:'+ _maxWidth +'px">';
                        Calendar.content += '<div class="cell-inner" data-time-slot="' + this.formatTime(t) + '"></div>';
                        var timeCurrent = t;

                        for (var s = 1; s < _slotCount; s++){
                            timeCurrent += Calendar.settings.widthTime
                            Calendar.content += '<div class="cell-inner" data-time-slot="'+ this.formatTime(timeCurrent) +'"></div>';
                        }
        ...
        wView.prototype._loadDates = function() {
            ...
            for (var i = this.weekStart; i <= weekend; i++) {
                ...
                $("#viewTableHead").find(".thead .tcell:nth-child(" + column + ")")
                    .html('<center><span class="weekday">'+ moment(date).format('D') +'日(' + moment(date).format('dd') + ')</span></center>')
                    .attr('data-day', moment(date).format('YYYY-MM-DD'));
                column++;
            }
            calendar.startOfWeekDate = moment([calendar.year, calendar.month, calendar.date]).startOf('isoWeek');
        ...
        wView.prototype.renderEvent = function(eventStartHours, eventStartMins, eventEndHours, evenEndMins, cellNo, arrayIndex, eventDuration) {
            ...
            var slotNumber = (Math.round(eventStartMins / Calendar.settings.slotDefault) * Calendar.settings.slotDefault);
            this.snapGridHeight = $('.cell-inner').outerHeight().toFixed(2);

            if(slotNumber == 60 ){
                eventStartHours = parseInt(eventStartHours) + 1;
                slotNumber = 0;
            }
            slotNumber = slotNumber  / Calendar.settings.slotDefault;
            ...
            if(slotParent.length <= 0){
                slotParent = $($(".tble .cell-inner[data-time-slot='"+eventStartHours+":30']")[cellNo]).parent();
                slotNumber = slotNumber - 6;
            }

            var duration = moment(Calendar.settings.events[arrayIndex].end).diff(Calendar.settings.events[arrayIndex].start, 'minutes')
            duration = Math.round(duration/Calendar.settings.slotDefault) * Calendar.settings.slotDefault;

            var remainingMins = duration;
            var height;
            while(remainingMins > 0){
                if( remainingMins > this.maxMinsPerDay){
                    height = (MAXMINS / Calendar.settings.slotDefault) * this.snapGridHeight;
                    remainingMins = remainingMins - MAXMINS;
                    remainingMins = (Math.round(remainingMins / Calendar.settings.slotDefault) * Calendar.settings.slotDefault);
                    ...
                    height = (remainingMins / Calendar.settings.slotDefault) * this.snapGridHeight;
                    remainingMins = 0;
                    $(slotParent.children()[slotNumber]).append(this._buildEventElement(height,arrayIndex,cellNo,eventStartHours, eventStartMins, eventEndHours, evenEndMins));
                }
            ...
            wView.prototype._buildEventElement = function(h,arrayIndex,cellNo,eventStartHours, eventStartMins, eventEndHours, evenEndMins){
                ...
                var styleConfig = Calendar.settings.slotDuration <= 30 ? 'float: left' : '';
                var styleConfigCount = Calendar.settings.slotDuration <= 30 ? 'float: right' : '';
                var readonly = (Calendar.settings.events[arrayIndex].readOnly == true) ? "readonly":"";
                var title = moment(Calendar.settings.events[arrayIndex].start).format('HH:mm') + " - "
                    + moment(Calendar.settings.events[arrayIndex].end).format('HH:mm') + "\n"
                    + Calendar.settings.events[arrayIndex].quantity + " / " + Calendar.settings.events[arrayIndex].total + "名"
                    + "\n" + Calendar.settings.events[arrayIndex].title;
                var eventContent = "<div data-toggle='tooltip' title='"+ title +"' class='event-container " + Calendar.settings.events[arrayIndex].class+ " "+readonly+"' data-event-duration=" + moment(Calendar.settings.events[arrayIndex].end).diff(Calendar.settings.events[arrayIndex].start, 'minutes')  + " data-index=" + arrayIndex + " data-startTime=" + Calendar.settings.events[arrayIndex].start + " data-endTime=" + Calendar.settings.events[arrayIndex].end + " id=" + id + " data-id=" + id + " data-row=" + eventStartHours + " data-cell=" + cellNo + " style=" + h + ">"
                eventContent += "<div class='event-inner'>";
                eventContent += "<div class='event-block-time' style='"+ styleConfig +"'>";
                eventContent += "<div class='time-wrap'><span class='event-start-time'>" + moment(Calendar.settings.events[arrayIndex].start).format('HH:mm') + "</span> - ";
                eventContent += "<span class='event-end-time'>" + moment(Calendar.settings.events[arrayIndex].end).format('HH:mm') + "</span></div>";
                eventContent += "<div class='event-title'>" + Calendar.settings.events[arrayIndex].title + "</div>";
                eventContent += "</div>";
                if(typeof Calendar.settings.events[arrayIndex].rest_time == 'undefined'){
                    eventContent += "<div class='event-quantity' style='"+ styleConfigCount +"'><span class='quantity'>" + Calendar.settings.events[arrayIndex].quantity + "</span> / <span class='total'>" + Calendar.settings.events[arrayIndex].total +"名</span></div>";
                }
                eventContent += "</div>"
        ...
        wView.prototype._timeslots =  function(){
            ...
            for (let t = startTime; t < endTime; t += Calendar.settings.widthTime) {
                if((before_time < 0) || (Math.floor(before_time / 3600) != Math.floor(t / 3600))){
                    //Dùng để tính toán số phần trong 1 khung giờ. Để tạm, sau sẽ để setting động.
                    let minutes = this.formatTime(t);
                    let maxWith = 80;
                    let minutesGet = minutes.split(':');
                    let minutesTimeLimit = Calendar.settings.maxTime.split(':');
                    ...
                    before_time = t;
                    Calendar.content += '<div class="time-slot" style="height:'+ maxWith +'px"><span>' + this.formatTime(before_time) + '</span></div>';
                }
            }

            // for (var i = Calendar.settings.minTime; i < Calendar.settings.maxTime; i++) {
            //     let hour = moment().hour(i).format('HH:00');
            //     let hourNext = moment().hour(parseInt(i) + 1).format('HH:00');
            //     Calendar.content += '<div class="time-slot"><span>' + `${hour}<br/>～<br/>${hourNext}` + '</span></div>';
            // }
            $(container).append(Calendar.content);
        }

        wView.prototype.calcStringTime = function (string) {
        ...
        wView.prototype.formatTime = function (min) {
        ...
        wView.prototype._setActive = function() {
            ...
            var date = moment([calendar.year, calendar.month, calendar.date]).format("YYYY-MM-DD");
        ...
        Calendar = {
            ...
            Init: function(rebuild) {
                ...
                if (Calendar.settings.dateStartWeek != null) {
                    let parseMoment = moment(Calendar.settings.dateStartWeek);
                    ...
                    calendar.month = parseMoment.month();
                    calendar.year = parseMoment.year();
                    calendar.date = parseMoment.format("D");

                    calendar.dayOfWeek = parseMoment.isoWeekday();
                    calendar.monthLong = moment(plugin.now).format('MMM');
                } else {
                    calendar.month = moment().month();
                    calendar.year = moment().year();
                    calendar.date = moment().format("D");
                    calendar.dayOfWeek = moment().isoWeekday();
